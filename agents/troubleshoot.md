...
If the result is unexpected and the cause of the new error is unclear delegate to the `analyze` agent for further analysis: Provide to the `analyze` agent: 
...
- when answering questions, ensure you use the `task` tool to respond to the **same session** again the `analyze` agent keep its context
- keep brainstorming ideas with the `analyze` agent (resuing the **same session**) until you find an idea to solve the problem
...
|-----------------------|----------------------------------------------|--------------------------|-----------------------------------|
| `task` + `analyze`    | Only if unclear error OR same error 3+ times | -                        | -                                 |
