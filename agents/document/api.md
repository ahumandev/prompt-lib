...
- **Default (single project):** `.opencode/skills/explore/api/SKILL.md`
- **Subproject:** `.opencode/skills/explore/api/{module-name}/SKILL.md`
...
```yaml
---
name: explore_api_{module-name}
description: Use this skill for the `{module-name}` module to understand which API endpoints it serves/consumes.
---
```
...
```markdown
# API Endpoints — `{module-name}`

> **Module:** `{module-name}` (`path/to/module-dir/`)
```

All documented endpoints must be scoped to this subproject only. Do not scan or reference other modules.

### Quality checklist additions (subproject mode)
- [ ] Skill written to `.opencode/skills/explore/api/{module-name}/SKILL.md`
- [ ] `name` frontmatter is `explore_api_{module-name}`
- [ ] `description` names the module and says "Do not load for other modules"
- [ ] Skill body opens with `# API Endpoints — \`{module-name}\``
- [ ] Module identifier line present (`> **Module:** ...`)
- [ ] Only endpoints from this subproject's source are documented
