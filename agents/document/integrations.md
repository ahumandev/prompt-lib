...
- **Default (single project):** `.opencode/skills/explore/integrations/SKILL.md`
- **Subproject:** `.opencode/skills/explore/integrations/{module-name}/SKILL.md`
...
```yaml
---
name: explore_integrations_{module-name}
description: Use this skill for the `{module-name}` module to understand its external integrations.
---
```
...
```markdown
# External Integrations — `{module-name}`

> **Module:** `{module-name}` (`path/to/module-dir/`)
```

All documented integrations must be scoped to this subproject only. Do not scan or reference other modules.

### Quality checklist additions (subproject mode)
- [ ] Skill written to `.opencode/skills/explore/integrations/{module-name}/SKILL.md`
- [ ] `name` frontmatter is `explore_integrations_{module-name}`
- [ ] `description` names the module and says "Do not load for other modules"
- [ ] Skill body opens with `# External Integrations — \`{module-name}\``
- [ ] Module identifier line present (`> **Module:** ...`)
- [ ] Only integrations from this subproject's source are documented
